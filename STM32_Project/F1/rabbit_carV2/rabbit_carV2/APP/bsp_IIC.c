...
static ACK_Value_t IIC_Write_Byte(IIC_Parameter_st *dev, uint8_t WR_Byte);
static uint8_t IIC_Read_Byte(IIC_Parameter_st *dev, ACK_Value_t ACK_Value);
...
static ACK_Value_t IIC_Write_Byte(IIC_Parameter_st *dev, uint8_t WR_Byte)
{
	...
	ACK_Rspond = (ACK_Value_t)READ_SDA(dev->SDA_GPIOx, dev->SDA_GPIO_Pin); // 读取此时的 SDA 值，即为从机的应答值
...
static uint8_t IIC_Write_Buffer(IIC_Parameter_st *dev, uint8_t addr, uint8_t reg, uint32_t len, uint8_t *data)
{
    ...
    bsp_IIC.IIC_Write_Byte(dev, addr | 0x00);	// 写操作
    bsp_IIC.IIC_Write_Byte(dev, reg);	// 发送寄存器地址
