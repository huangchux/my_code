...
static void Motor_Init(void)
{
    ...
    Motor.Motor_Set_PWM(Motor_Left, 0);       // 左电机默认占空比0
    Motor.Motor_Set_PWM(Motor_Right, 0);      // 右电机默认占空比0
...
static void Motor_Set_PWM(Motor_Switch_et Motor, int16_t duty)
{
    ...
    if (Motor_Left == Motor)  // 如果是左边的电机
    {
        ...
        if (duty < 0)  // 如果传入参数为负
        {
            ...
            TIM1->CCR1 = (duty + 100); // 设置占空比 这里要+100 因为写入寄存器必须为正数 值越大速度越快
        }
        else  // 如果传入参数为正
        {
    ...
    else if(Motor_Right == Motor)  // 如果是右边的电机
    {
        ...
        if (duty < 0)  // 如果传入参数为负
        {
            ...
            TIM1->CCR4 = (duty + 100); // 设置占空比 这里要+100 因为写入寄存器必须为正数 值越大速度越快
        }
        else  // 如果传入参数为正
        {
            Motor_Set_Direction(Motor, Motor_Forward);  // 正向
