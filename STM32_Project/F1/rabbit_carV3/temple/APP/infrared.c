...
static void PID_Control_HW_Tracking(void)
{
    ...
    else if((1 == Infrared.HW_Read_OUT[0]) && (0 == Infrared.HW_Read_OUT[1]) && (0 == Infrared.HW_Read_OUT[2]) && (1 == Infrared.HW_Read_OUT[3]))
    {
        This_state = 2; // 快速左转
    }
    else if((1 == Infrared.HW_Read_OUT[0]) && (0 == Infrared.HW_Read_OUT[1]) && (1 == Infrared.HW_Read_OUT[2]) && (1 == Infrared.HW_Read_OUT[3]))
    {
        This_state = 3; // 快速左转
    }

    PID_HW_Tracking.PWM_out = PID.PID_Control(&PID_HW_Tracking, This_state);  // PID计算输出目标速度
