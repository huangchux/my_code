...
static void Menu_Handler(void)
{
    switch(Menu_Status)
    {
        case Page_Power:  // 选择页面
        {
            Menu_Display_1(); // 电量显示页面
            break;
        }
...
static void Menu_Display_1(void)
{
    ...
    OLED.OLED_Show_String(0, OLED096_Line2, (char *)arr, ASCII_SIZE_16); // 将电压值显示在OLED的第二行上
...
static void Menu_Display_3(void)
{
    ...
    OLED.OLED_Show_String(0, OLED096_Line2, "    L ----- R  ", ASCII_SIZE_16); // 第一行
